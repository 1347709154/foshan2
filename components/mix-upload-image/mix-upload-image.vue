<template>
	<view class="upload-content"  >
		<view  class="upload-item" v-if="image.file_path">
			<image class="upload-img" :src="image.file_path" mode="aspectFill" @click="previewImage(index)"></image>
			<!-- 删除按钮 -->
...
export default {
	...
	methods: {
		...
		uploadFile: function(filePath) {
			...
			uni.uploadFile({
				url: 'https://xx.gzlsf.cn/index.php?s=/api/upload/image',
				filePath: filePath[0],
				name: 'iFile',
				formData: {
					wxapp_id: 10001,
		...
		async uploadFiles(files){
			...
			if(tempFile.code === 'SUCCESS'){
				item.url = tempFile.download_url;
				this.$emit('onChange', {
					list: this.imageList,
					index: this.index
				})
		...
		//预览图片
		previewImage(index){
			const urls = this.image.file_path;
			uni.previewImage({
				current: index,
				urls: urls
			})
