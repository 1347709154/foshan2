<template>
	<view class="content">
		<view class="card">
			...
			<view class="card-name">{{detail.name}}</view>
...
<script>
	export default {
		...
		methods: {
			getList: function() {
				this.$requests('article/order', {
					article_id: this.article_id
				}).then(res => {
					this.detail = res.detail.category;
					this.total_price = res.detail.category.price;
			...
			submit() {
				...
				this.$requests('article/submitOrder', {
					time,
					article_id,
					number,
					pay_type
