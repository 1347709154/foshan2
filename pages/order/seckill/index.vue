<template>
	<view class="content">
		...
		<view class="nav">
			...
			<view @click="change(2)" class="nav-ul"><text :class="[btnnum==2?'active':'text']">秒杀成功</text></view>
			<view @click="change(3)" class="nav-ul"><text :class="[btnnum==3?'active':'text']">秒杀失败</text></view>
		...
		<view class="order-box">
			<view class="order-list" v-for="(item,index) in order" :key="index">
				<view class="order-list-top">
					...
					<view class="order-state" style="color: #FF0000;" v-if="item.state==2">秒杀成功</view>
					<view class="order-state" style="color: #FF0000;" v-if="item.state==3">秒杀失败</view>
