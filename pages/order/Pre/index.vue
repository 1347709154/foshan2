	...
	<view class="content">
		...
		<view class="nav">
			...
			<view @click="change('received')" class="nav-ul"><text
					:class="[btnnum=='received'?'active':'text']">预约成功</text></view>
			...
			<view class="order-list" v-for="(item,index) in list" :key="index">
				...
				<view class="order-list-zj">
					<view class="order-list-goodsimg">
						<image :src="item.article.image[0].file_path" mode="aspectFill"></image>
					</view>
					<view class="order-list-desc">
						<view>{{item.article.article_title}}</view>
				...
				<view class="order-list-bottom" v-if="item.pay_status==10 && item.order_status.value==10">
					<view class="gaiqian" style="border: 1rpx solid #E2E1E1;color: #131313;" @click="cancelOrder(item.order_id)">取消订单
					</view>
