	...
	<view class="content">
		...
		<view class="nav">
			...
			<view @click="change('received')" class="nav-ul"><text
					:class="[btnnum=='received'?'active':'text']">待使用</text></view>
			...
			<view class="order-list" v-for="(item,index) in list" :key="index">
				<view class="order-list-top">
					<view class="order-img">
						<image src="../../../static/user/jd.png"></image>
						<view v-if="item.hotel">{{item.hotel.hotel_name}}</view>
					</view>
				...
				<view class="order-list-zj">
					<view class="order-list-goodsimg">
						<image v-if="item.house" :src="item.house.image[0].file_path" mode="aspectFill"></image>
					</view>
					<view class="order-list-desc">
						<view v-if="item.house">{{item.house.house_name}} x1晚</view>
						<view>入住时间：{{item.time}}</view>
