<template>
	<view class="content">
		...
		<view class="listitem" v-for="item in list" @click="ToDetail(item.article_id)">
			<view class="topimg">
				<image :src="item.image[0].file_path" mode="aspectFill" class="timg"></image>
				<view class="readnum">
					<image  src="../../static/scenic/eye.png" mode="" class="left eye"></image>
					<span>已有</span><span>{{item.show_views}}</span><span>人阅读</span>
				</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			getList: function() {
				this.$requests('column/lists', {column_id:2,key:this.key}).then(res => {
					this.list = res.list;
				})
