<template>
	<view class="wrap">
		<view class="top">
			<view class="item">
				<view class="left">收货人</view>
				<input type="text" v-model="addr.name" placeholder-class="line" placeholder="请填写收货人姓名" />
			</view>
			<view class="item">
				<view class="left">手机号码</view>
				<input type="text" v-model="addr.phone" placeholder-class="line" placeholder="请填写收货人手机号" />
			</view>
			<view class="item" @tap="showRegionPicker">
				<view class="left">所在地区</view>
				<input disabled type="text" placeholder-class="line" placeholder="省市区县、乡镇等" v-model="region" />
			</view>
			<view class="item address">
				<view class="left">详细地址</view>
				<textarea type="text" v-model="addr.detail" placeholder-class="line" placeholder="街道、楼牌等" />
			</view>

		</view>

		<u-picker mode="region" ref="uPicker" :params="params" @confirm="confirm" v-model="show" />
		<view class="bt1" @click="deAddr">删除</view>
		<view class="bt" @click="submit">保存</view>
	</view>
</template>

<script>
	export default {
		data() {
			return {
				address_id: '',
				name: '',
				phone: '',
				address: '',
				show: false,
				checked: false,
				region: '',
				params: {
					province: true,
					city: true,
					area: true
				},
				addr:{}
			};
		},
		onLoad(options) {
				this.address_id=options.address_id;
				this.getAddressDetail(options.address_id);
		},
		methods: {
			getAddressDetail: function(address_id) {
				let _this = this;
				this.$requests('address/detail', {
					address_id
				}).then(res => {
					_this.addr=res.detail;
					_this.region=res.region;
				});
			},
			deAddr(){
				this.$requests('address/delete', {
					address_id:this.address_id
				}).then(res => {
					uni.navigateTo({
						url:'./index'
					})
				});
			},

			submit() {
				const {
				addr
				} = this;
				if (!addr.name) {
					this.$util.msg('请输入姓名');
					return;
				}
				if (!addr.phone && addr.phone < 11) {
					this.$util.msg('请输入手机号');
					return;
				}

				if (!addr.region) {
					this.$util.msg('请选择地址');
					return;
				}
				if (!addr.detail) {
					this.$util.msg('请输入详细地址');
					return;
				}
				this.$requests('address/edit', {
					addr
				}, 'POST').then(res => {
					this.$util.msg('更新成功');
					setTimeout(() => {
						uni.navigateBack({

						})
					}, 1000)
				});
			},



			confirm(e) {
				this.addr.region = e.province.label + '-' + e.city.label + '-' + e.area.label;
				this.region = e.province.label + '-' + e.city.label + '-' + e.area.label;
			},
			//默认地址开关
			setDefault: function(e) {
				console.log(e.target.value);
				this.checked = e.target.value
			},
			showRegionPicker() {
				this.show = true;
			}
		}
	};
</script>

<style lang="scss" scoped>
	/deep/ .line {
		color: $u-light-color;
		font-size: 28rpx;
	}

	.bt {

		position: absolute;
		bottom: 360rpx;
		background: #07C160;
		color: #ffffff;
		text-align: center;
		height: 80rpx;
		line-height: 80rpx;
		width: 80%;
		border-radius: 40rpx;
		margin-left: 10%;
	}

	.bt1 {

		position: absolute;
		bottom: 200rpx;
		background: #ea0000;
		color: #ffffff;
		text-align: center;
		height: 80rpx;
		line-height: 80rpx;
		width: 80%;
		border-radius: 40rpx;
		margin-left: 10%;
	}

	.wrap {

		background-color: #f2f2f2;

		.top {
			background-color: #ffffff;
			border-top: solid 2rpx $u-border-color;
			padding: 22rpx;

			.item {
				display: flex;
				font-size: 32rpx;
				line-height: 100rpx;
				align-items: center;
				border-bottom: solid 2rpx $u-border-color;

				.left {
					width: 180rpx;
				}

				input {
					text-align: left;
				}
			}

			.address {
				padding: 20rpx 0;

				textarea {
					// width: 100%;
					height: 150rpx;
					background-color: #f7f7f7;
					line-height: 60rpx;
					margin: 40rpx auto;
					padding: 20rpx;
				}
			}

			.site-clipboard {
				padding-right: 40rpx;

				textarea {
					// width: 100%;
					height: 150rpx;
					background-color: #f7f7f7;
					line-height: 60rpx;
					margin: 40rpx auto;
					padding: 20rpx;
				}

				.clipboard {
					display: flex;
					justify-content: center;
					align-items: center;
					font-size: 26rpx;
					color: $u-tips-color;
					height: 80rpx;

					.icon {
						margin-top: 6rpx;
						margin-left: 10rpx;
					}
				}
			}
		}

		.bottom {
			margin-top: 20rpx;
			padding: 40rpx;
			padding-right: 0;
			background-color: #ffffff;
			font-size: 28rpx;

			.tag {
				display: flex;

				.left {
					width: 160rpx;
				}

				.right {
					display: flex;
					flex-wrap: wrap;

					.tags {
						width: 140rpx;
						padding: 16rpx 8rpx;
						border: solid 2rpx $u-border-color;
						text-align: center;
						border-radius: 50rpx;
						margin: 0 10rpx 20rpx;
						display: flex;
						font-size: 28rpx;
						align-items: center;
						justify-content: center;
						color: $u-content-color;
						line-height: 1;
					}

					.plus {
						//padding: 10rpx 0;
					}
				}
			}

			.default {
				margin-top: 50rpx;
				display: flex;
				justify-content: space-between;
				border-bottom: solid 2rpx $u-border-color;
				line-height: 64rpx;

				.tips {
					font-size: 24rpx;
				}

				.right {}
			}
		}
	}
</style>
